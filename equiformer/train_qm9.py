...
def main(args):
    ...
    # initialize wandb
    if args.log:
        wandb.init(project="Equiformer_qm9_" + f"target{args.target}", name=args.ID, config=args)
    ...
    for epoch in range(args.epochs):
        ...
        train_err = train_one_epoch(model=model, criterion=criterion, norm_factor=norm_factor,
            ...
            print_freq=args.print_freq, logger=_log, l1_weight=args.l1_weight, log=args.log)
        ...
        if args.log:
            wandb.log({"val error": val_err, "test error": test_err})
        ...
        if val_err < best_val_err:
            ...
            best_epoch = epoch
            # save best validated model dict
            save_dir = "saved_models"
            os.makedirs(save_dir, exist_ok=True)
            torch.save(model.state_dict(), os.path.join(save_dir, "equiformer_" + args.ID + ".pt"))
    ...
    # best validated test MAE
    if args.log:
        state_dict = torch.load(os.path.join(save_dir, "equiformer_" + args.ID + ".pt"))
        model.load_state_dict(state_dict)
        test_err, test_loss = evaluate(model, norm_factor, args.target, test_loader, device, 
                                    amp_autocast=amp_autocast, print_freq=args.print_freq, logger=_log)
        wandb.log({"best validated test error": test_err})
